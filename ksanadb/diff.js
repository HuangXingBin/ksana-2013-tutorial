...
change it to require.js module
*/
...
define([],function () {
  ...
  diff_match_patch.prototype.diff_bisect_ = function (a, b, c) {
    ...
    for (var k = d - e, p = 0 != k % 2, q = 0, s = 0, o = 0, v = 0, u = 0; u < f && !((new Date).getTime() > c); u++) {
      ...
      for (n = -u + o; n <= u - v; n += 2) {
        ...
        else if (!p && (l = g + k - n, 0 <= l && l < h && -1 != j[l] && (m = j[l], r = g + m - l, t = d - t, m >= t))) return this.diff_bisectSplit_(a, b, m, r, c)
      }
    }
    return [[-1, a], [1, b]]
  };
  ...
  diff_match_patch.prototype.match_bitap_ = function (a, b, c) {
    ...
    for (var j = 1 << b.length - 1, h = -1, i, k, p = b.length + a.length, q, s = 0; s < b.length; s++) {
      i = 0;
      for (k = p; i < k;) d(s, c + k) <= g ? i = k : p = k, k = Math.floor((p - i) / 2 + i);
      p = k;
      i = Math.max(1, c - k + 1);
      ...
      for (k[o + 1] = (1 << s) - 1; o >= i; o--) {
        ...
        if (k[o] & j && (v = d(s, o - 1), v <= g)) if (g = v, h = o - 1, h > c) i = Math.max(1, 2 * c - h);
        else break
      }
      if (d(s + 1, c) > g) break;
      q = k
    ...
    for (var d = 0, e = [], f = 0; f < a.length; f++) {
      ...
      if (-1 == j) e[f] = !1, d -= a[f].length2 - a[f].length1;
        ...
        for (var h = 0, k, i = 0; i < a[f].diffs.length; i++) {
          ...
          1 === p[0] ? b = b.substring(0, j + k) + p[1] + b.substring(j + k) : -1 === p[0] && (b = b.substring(0, j + k) + b.substring(j + this.diff_xIndex(g, h + p[1].length))); - 1 !== p[0] && (h += p[1].length)
        }
      }
    }
  ...
  diff_match_patch.diff_match_patch = diff_match_patch;
